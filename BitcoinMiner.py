...
class BitcoinMiner():
	def __init__(self, device, host, user, password, port=8332, frames=30, rate=1, askrate=5, worksize=-1, vectors=False, verbose=False, logger=None, statushandler=None, blkfound=None):
	...
	def failure(self, message):
		print '\n%s' % message
		self.logger and self.logger.error(message)
		self.exit()
	...
	def mine(self):
		...
		while True:
			...
			try:
				...
				self.logger and self.logger.error(exc)
				print exc
				...
				if h[7] != 0:
					...
					if belowOrEquals(h[:7], result['target'][:7]):
						...
						if accepted != None:
							...
							self.blockFound(pack('I', long(h[6])).encode('hex'), accepted, diff)
